...
The `navigator_up`, `navigator_down`, and `navigator_filter_*` fields customize the session navigator (`prefix+g`) and may also use plain keys such as `j` or `k`. They are navigator-mode-only and must not use `prefix+` or `esc`. Up and down arrows are permanent aliases for navigator movement; the filter keys apply only while the navigator list is focused, not while searching. The navigator footer shows the active bindings.
